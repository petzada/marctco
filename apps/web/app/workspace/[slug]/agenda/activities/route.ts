...
interface CreateAgendaActivityBody {
...
export async function POST(
  ...
  let body: CreateAgendaActivityBody;
  try {
    body = (await request.json()) as CreateAgendaActivityBody;
  } catch {
    return NextResponse.json({ error: "Pedido inválido." }, { status: 400 });
  }

  if (!body.opportunity_id) {
    return NextResponse.json({ error: "Escolha o lead desta atividade." }, { status: 400 });
  }
