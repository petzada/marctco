...
| **3 · Tempo · entregue** | **Activity** (`due_at`, tipo, responsável) + SLA desde `arrived_at` + estagnação + **Agenda** + alerta ao gestor + **Dashboard operacional** | — |
| **4 · Canal · entregue** | WhatsMiau + template de 1º contato + timeline no card | [0003](./adr/0003-whatsapp-instancia-unica-gatilho-atribuicao.md) |
...
| A5 | **Fechado pela spec da Fase 4 e materializado pelo ticket 00:** WhatsMiau é tratado como gateway não-oficial; permanecem default `ON_ASSIGNMENT`, opt-in explícito, delay e rate limit. Se o contrato oficial do provedor mudar, a porta de mensageria permite revisitar a premissa sem trocar o domínio — [.scratch/canal/spec.md](https://github.com/petzada/marctco/blob/docs/arquivo-fases-0-4/.scratch/canal/spec.md) · [ADR-0003](./adr/0003-whatsapp-instancia-unica-gatilho-atribuicao.md) | Fechado |
...
| A10 | **Endereçado pelo ticket 02**, que cria o arquivo de tokens referenciado por `{token.refs}` e registra as lacunas conhecidas em comentário. Paleta de dataviz **fechada pelo ticket 08** da Fase 3 (`DESIGN.md` "Colors > Data visualization"); Analytics herda pronta. Fica aberto só motion e densidade em uma altura só | Ticket 02 · dataviz no ticket 08 |
