...
| Opt-in de WhatsApp (snapshot) | `Opportunity.whatsapp_opt_in` | Snapshot operacional copiado da submissão que criou ou liberou a Oportunidade. Disparo automático exige `true`; ausente, falso ou ilegível falha fechado |
...
| Opt-in de WhatsApp (evidência) | `LeadSubmission.whatsapp_opt_in` | `boolean \| null`. O que o contrato `v1` recebeu. Conector traduz o campo da origem; não infere de telefone, campanha ou provedor |
...
| Contrato canônico de entrada | `InboundLead` | O contrato `v1` já interpretado, com `source` e `external_lead_id` resolvidos. Inclui `whatsapp_opt_in: boolean \| null`. Zod é a fonte única e o tipo é inferido ([ADR-0008](./0008-fronteira-conector-dominio.md)). **Nunca** `LeadPayload` como tipo de domínio |
...
| Evento da linha do tempo da Oportunidade | `OpportunityTimelineEvent` | Fato imutável. Ingestão: `RETRANSMISSION_RECEIVED \| SUBMISSION_REENTERED`. Movimento (Fase 3 ticket 04): `STAGE_CHANGED \| ASSIGNED \| REASSIGNED \| RETURNED_TO_QUEUE \| ACTIVITY_CREATED \| ACTIVITY_COMPLETED`. Canal (Fase 4): `WHATSAPP_OUTBOUND_SENT \| WHATSAPP_OUTBOUND_FAILED \| WHATSAPP_INBOUND_RECEIVED`. Fato de movimento ou de mensagem não tem evento de integração e não deduplica pelo outbox de ingestão |
| Handoff | `Handoff` | — |
...
| Conexão de integração | `IntegrationConnection` | Um provedor admite N no mesmo workspace ([ADR-0031](./0031-conexao-na-chave-idempotente.md)). WhatsMiau é a exceção pontual: no máximo uma não desligada por workspace ([ADR-0003](./0003-whatsapp-instancia-unica-gatilho-atribuicao.md)) |
...
| Superfície de integração | `IntegrationSurface` | Tipo da camada web, não model: liga o segmento de URL da tela ao `IntegrationProvider` que ela administra, para que as duas não voltem a divergir. A gaveta WhatsApp administra `WHATSMIAU` |
...
| Gatilho do 1º contato | `WorkspaceSettings.first_contact_trigger: ON_ASSIGNMENT \| ON_ARRIVAL \| DISABLED` | Configuração do gestor ([ADR-0003](./0003-whatsapp-instancia-unica-gatilho-atribuicao.md)). Default de domínio `ON_ASSIGNMENT`; ausência de linha não é `DISABLED` |
| Template de primeiro contato | `WorkspaceSettings.first_contact_template_body` | Texto com placeholders `{{snake_case}}`. Variáveis de atendente só no gatilho `ON_ASSIGNMENT` |
...
| SLA de primeiro contato | `WorkspaceSettings.first_contact_sla_minutes` | Minutos até a primeira evidência de atendimento. Anulável: nulo (ou linha ausente) resolve para o padrão do domínio, nunca desliga o relógio. Ortogonal a `first_contact_trigger` |
| Limite de estagnação | `WorkspaceSettings.stagnation_days` | Dias sem movimento. Anulável pela mesma regra do SLA |
| Primeiro contato | `Opportunity.first_contact_at` | Instante da primeira evidência de atendimento. Anulável; escrito uma vez com `WHERE first_contact_at IS NULL`. Evidência: primeira Atividade concluída, HTTP 2xx do `sendText` (política local de aceite, não entrega/leitura) ou resposta inbound. Atribuir, enfileirar ou falhar **não** preenchem |
...
| Tipo do fato de movimento | `OpportunityTimelineEventType` | `STAGE_CHANGED` (mover etapa), `ASSIGNED` (atribuir), `REASSIGNED` (reatribuir), `RETURNED_TO_QUEUE` (devolver à fila no desatrelamento), `ACTIVITY_CREATED` (marcar atividade nova — movimento sem ser atendimento), `ACTIVITY_COMPLETED` (concluir atividade). Canal: `WHATSAPP_OUTBOUND_SENT`, `WHATSAPP_OUTBOUND_FAILED`, `WHATSAPP_INBOUND_RECEIVED` |
...
| Origem do job | `JobOrigin` | União discriminada `{ type: "integration_event", integration_event_id } \| { type: "scheduled_sweep", sweep: ScheduledSweepName } \| { type: "channel_outbound", attempt_id } \| { type: "channel_inbound", integration_connection_id }`. O `type` evita colidir com o `kind` do `AccessContext` (`"user" \| "job"`). Evento de integração é linha real do tenant; passada agendada é nome fechado; canal aponta para tentativa ou conexão autenticada — nunca âncora fabricada ([ADR-0016](./0016-contexto-de-acesso-e-leitor-escopado.md)) |
...
| Descoberta de tentativas outbound pendentes | `private.claim_pending_channel_attempts` | Sétima função da lista fechada ([ADR-0019](./0019-resolucao-pre-contexto-e-executor-privado.md)). Chamada pelo dispatcher do app. Devolve somente `(attempt_id, workspace_id)`. Sem PII, telefone ou corpo de mensagem |
| Provedor WhatsMiau | `IntegrationProvider = … \| WHATSMIAU` | Valor novo no enum de provedor. A `apikey` é da conta Whatsmiau, server-side da marctco; o workspace persiste instância e hash de webhook. **Nunca** `WHATSAPP` no enum — o rótulo da gaveta é UI |
| Nome da instância WhatsMiau | `IntegrationConnection.instance_name` | O `instanceName` público de todas as rotas v2. Único na conta Whatsmiau; o CRM deriva um valor estável e globalmente único do workspace |
| Estado de pareamento | `WhatsAppPairingState: DISCONNECTED \| CONNECTING \| QR_PENDING \| CONNECTED \| SUSPENDED \| ERROR` | Normalização local, distinta de `IntegrationConnectionStatus`. Oficiais: `open → CONNECTED`, `closed → DISCONNECTED`, `connecting → CONNECTING`, `qr-code → QR_PENDING`, `suspended: true → SUSPENDED`. Webhook `close` também `DISCONNECTED`. `ERROR` é falha HTTP, payload inválido ou estado desconhecido — não valor da API |
| Tentativa outbound | `ChannelOutboundAttempt` | Uma mensagem automática por Oportunidade. É o outbox Postgres. Identidade do job: `attempt_id` |
| Estado de publicação | `ChannelOutboundAttempt.dispatch_status: PENDING \| DISPATCHED` | Publicação na fila, recuperável antes do HTTP. Distinto do estado de entrega |
| Estado de entrega | `ChannelOutboundAttempt.delivery_status: QUEUED \| PROCESSING \| SENT \| FAILED` | Resultado local do envio. `SENT` é HTTP 2xx de `sendText` (política local de aceite), **não** entrega nem leitura. Depois de `PROCESSING`, não há segundo `sendText` |
| Fato de mensagem na linha do tempo | `OpportunityTimelineEventType = … \| WHATSAPP_OUTBOUND_SENT \| WHATSAPP_OUTBOUND_FAILED \| WHATSAPP_INBOUND_RECEIVED` | Envio automático aceito; tentativa encerrada sem envio; resposta inbound. Copy PT-BR na UI; nunca “entregue” nem “lida” |
| Conexão do fato inbound | `OpportunityTimelineEvent.integration_connection_id` | A Conexão WhatsMiau que autenticou o webhook. Preenchido só em `WHATSAPP_INBOUND_RECEIVED`. Compõe a chave de dedupe com o identificador externo |
| Identificador externo da mensagem inbound | `OpportunityTimelineEvent.external_message_id` | `data.key.id` do `messages.upsert`. Dedupe `(workspace_id, integration_connection_id, external_message_id)` nas retentativas do provedor |
| Preview truncado da mensagem | `OpportunityTimelineEvent.message_preview` | Texto curto ou tipo genérico + caption; nunca o thread completo nem a mídia |
| Origem outbound do canal | `JobOrigin` variante `{ type: "channel_outbound"; attempt_id }` | Worker de envio. Constrói `JobContext` depois de resolver o workspace. Não fabrica `integration_event_id` |
| Origem inbound do canal | `JobOrigin` variante `{ type: "channel_inbound"; integration_connection_id }` | Webhook autenticado pelo hash já existente, que devolve `(workspace_id, integration_connection_id)`. O id da conexão é o da linha resolvida, nunca fabricado. Não nasce função privada só para autenticar |
