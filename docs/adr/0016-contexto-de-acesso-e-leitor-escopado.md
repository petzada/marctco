...
|---|---|---|---|
| `UserContext` | `apps/web` | `workspace_id`, `user_id`, `role` | validação do `slug` da URL contra `WorkspaceMember` ([ADR-0012](./0012-contexto-de-tenant-na-url.md)) |
...
**3. As três consultas sem tenant são a exceção, e ela já é fechada.** `resolve_workspace_by_token_hash`, `claim_pending_events` e `provision_workspace` acontecem **antes** de existir workspace para pôr num contexto — são justamente as que produzem o `workspace_id` com que o contexto é construído ([ADR-0006](./0006-rls-duas-camadas-guc-worker.md) regra 9). Elas não recebem `AccessContext` e não podem receber. A lista é fechada em três, o Seam 3 reprova qualquer `SECURITY DEFINER` fora dela, e nenhuma devolve payload. Sem esta cláusula escrita, a regra 2 pareceria ter um furo — e a quarta função entraria por ele.
