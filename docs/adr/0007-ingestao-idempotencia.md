...
> **Emendado pelo [ADR-0019](./0019-resolucao-pre-contexto-e-executor-privado.md):** a resolução de associação navegador → workspace é o quarto caso sem tenant. A lista fechada, o executor técnico `NOLOGIN` e as policies mínimas passam a ser definidos por aquele ADR — sete funções desde a emenda de 2026-08-19 da Fase 4; a redação original deste bloco falava no quarto caso. Esta decisão de ingestão permanece inalterada.
>
...
**Não é a única consulta sem tenant.** O dispatcher, que procura pendências de todos os workspaces sem sessão e sem job prévio, tem exatamente o mesmo formato — e um "claim por evento" seria circular, porque para setar o claim ele precisa do `workspace_id` do evento, e para ler o `workspace_id` ele precisaria do claim. O provisionamento e a resolução navegador → associação/workspace completam os outros casos desta decisão de ingestão. As funções sem tenant recebem o mesmo remédio e formam **lista fechada**, enumerada no [ADR-0019](./0019-resolucao-pre-contexto-e-executor-privado.md) — sete desde 2026-08-19 da Fase 4; a redação original deste parágrafo dizia quatro. O que cada uma devolve importa tanto quanto quem pode chamá-la: `claim_pending_events` devolve `(id, workspace_id)` e nunca o `raw`, que carrega CPF e telefone. `claim_pending_channel_attempts` devolve `(attempt_id, workspace_id)` e nunca telefone nem corpo de mensagem.
