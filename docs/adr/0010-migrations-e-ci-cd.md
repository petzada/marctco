...
> **Emendado pelo [ADR-0019](./0019-resolucao-pre-contexto-e-executor-privado.md):** o schema `private` hospeda a lista fechada de funções `SECURITY DEFINER` — sete desde a emenda de 2026-08-19 da Fase 4; a redação original deste bloco dizia quatro. A função de associação do navegador usa `marctco_private_definer`; cada função futura precisa de executor técnico compatível com `FORCE RLS`, sem congelar seu ownership antes da migration que a cria. A enumeração canônica vive no ADR-0019.
...
11. **Os papéis nascem dentro das migrations**, de forma idempotente e com nome prefixado (`marctco_migrator`, `marctco_app`, `marctco_worker`) para nunca colidir com os papéis internos do Supabase. **A senha nunca vai numa migration** — o arquivo está no git; a migration cria o papel e concede privilégios, e a senha é definida uma vez por `ALTER ROLE`, fora do versionamento.
    ...
    **Emenda de 2026-08-05 — papéis técnicos pós-foundation.** Papéis `NOLOGIN` criados por `postgres` no Supabase (por exemplo `marctco_private_definer`) exigem dois passos humanos antes do redeploy: `CREATE ROLE` quando `marctco_migrator` não tem `CREATEROLE`, e `GRANT … TO marctco_migrator WITH INHERIT FALSE, SET TRUE` porque só quem tem `ADMIN` no papel pode concedê-lo — membership existente não substitui um novo `GRANT` na migration. As migrations usam blocos idempotentes (`IF NOT EXISTS` no `CREATE ROLE` e consulta a `pg_auth_members` antes do `GRANT`) para que o redeploy pule o que o humano já fez; a recuperação fail-closed reconhece os dois erros conhecidos e aborta se faltar o bootstrap correspondente.

    **Emenda de 2026-08-05 — papéis técnicos `NOLOGIN` pós-foundation.** Depois que `MIGRATION_DATABASE_URL` passa a usar `marctco_migrator`, migrations que criam executores técnicos adicionais — como `marctco_private_definer` na `20260805000200_authentication_workspace_context` — falham com `permission denied to create role`, porque o migrator não tem `CREATEROLE`. O CI continua verde: o Postgres efêmero usa `postgres` com `CREATEROLE`. Em produção, o papel técnico precisa ser pré-criado uma vez no SQL Editor do Supabase, autenticado como `postgres`, **antes** do re-run do job de release. A migration shipada usa `IF NOT EXISTS` no `CREATE ROLE`; o recovery fail-closed só marca `rolled-back` quando o log bate com o erro conhecido, a função `private.resolve_user_workspaces` e as policies `workspaces_private_definer_select` / `workspace_members_private_definer_select` estão ausentes, e o papel `marctco_private_definer` já existe. Sem o papel pré-criado, o recovery aborta com a instrução SQL exata — nunca tenta criar o role nem prossegue cegamente.

    **Emenda de 2026-08-05 — ownership do schema `private` pós-foundation.** A foundation cria `private` antes de `SET ROLE marctco_migrator`, então em Supabase gerenciado o schema fica owned by `postgres`. A `20260805000200_authentication_workspace_context` roda como `marctco_migrator` e precisa `GRANT`/`REVOKE` no schema para o executor técnico; sem ownership, falha com `permission denied for schema private`. O CI não reproduz: o deploy inicial roda como `postgres`, que é owner. Em produção, após os bootstraps de `CREATE ROLE` e `GRANT … TO marctco_migrator`, o humano executa uma vez `ALTER SCHEMA private OWNER TO marctco_migrator` como `postgres`. A 002 shipada faz `SET ROLE marctco_migrator` antes dos grants de schema e consulta `has_schema_privilege` para pular grants já concedidos; o recovery fail-closed reconhece o terceiro erro conhecido e aborta enquanto `pg_namespace.nspowner` de `private` não for `marctco_migrator`.

    **Emenda de 2026-08-06 — bootstrap anunciado antes do release.** A `20260806000100_provision_workspace` cria o quarto papel técnico, `marctco_provisioner`, e cai na mesma parede: o migrator não tem `CREATEROLE`. Em vez de deixar o Postgres responder `permission denied to create role`, a migration verifica `rolcreaterole OR rolsuper` do usuário corrente e falha com o `CREATE ROLE`/`GRANT` exatos a executar no SQL Editor. A exceção acontece antes de qualquer DDL, então o rollback é limpo. O recovery automático de `pnpm db:recover:foundation` cobre apenas as migrations `001` e `002`; para esta, a retomada é `prisma migrate resolve --rolled-back 20260806000100_provision_workspace` seguida de novo `migrate deploy`, depois do bootstrap humano. **A ordem barata é a inversa: executar os dois comandos no Supabase antes de mesclar o PR**, e o release passa direto.
