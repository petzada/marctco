...
- **No disclosure surface for a single anchor point (popover/tooltip) is documented.** `{component.dropdown-menu}` covers a list of actions; `{component.modal}` covers a blocking task. Neither is specified for "float a small amount of content next to one element and let the user dismiss it without leaving the page" — the shape a lead's single markers-icon needs. The choice between reusing `{component.dropdown-menu}` for this or adding a new `{component.popover}` to this guide belongs to whichever ticket builds that surface first (currently ticket 12); it must be recorded here as a new component entry, not improvised inline where it's consumed.
