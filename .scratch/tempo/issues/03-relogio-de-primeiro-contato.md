...
- [x] O estado de SLA é **função pura** em `packages/domain`: recebe `arrived_at`, `first_contact_at`, situação, configuração resolvida e `now`, e devolve `PENDING | MET | BREACHED` com a duração
...
- [x] Lead `WON`/`LOST` sem nenhuma atividade concluída para de correr e **não** conta como atendido
...
- [x] A tabela de Leads e o card mostram a espera com numerais tabulares, como a parcela
