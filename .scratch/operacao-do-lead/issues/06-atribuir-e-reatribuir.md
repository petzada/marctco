...
- [x] Atribuir e reatribuir são operações **diferentes** ([ADR-0013](../../../docs/adr/0013-fluxo-de-dados-no-app.md)): atribuir exige `assigned_user_id` nulo no `WHERE`; reatribuir exige o dono atual no `WHERE`
- [x] `assignLead` já arbitra a corrida e já recusa Atendente — este ticket estreita o ator e o destino: só Gestão e Direção atribuem da fila ([ADR-0024](../../../docs/adr/0024-fila-sem-dono-e-da-gestao.md)); destino válido é `SUPERVISOR` `ACTIVE` **com ao menos uma tag** **ou** o próprio ator ([ADR-0025](../../../docs/adr/0025-destino-da-fila-e-supervisor-ou-ator.md)); recusa destino `DETACHED`, `ATTENDANT`, Supervisor sem tag, outro `MANAGER`, `OWNER` que não seja o ator; recusa `SUPERVISOR` e `ATTENDANT` como atores
...
- [x] A tabela permite selecionar vários leads e atribuir/reatribuir o lote a **um** destino — mesma regra do 1 a 1, N linhas ([ADR-0026](../../../docs/adr/0026-atribuicao-em-massa.md)). Não rateia. 1 a 1 permanece. **Lote parcial:** quem ainda podia ir, vai; quem já tinha dono recusa pelo **nome** e permanece na tabela; last-write-wins continua proibido
...
- [x] Remoção otimista da linha atribuída via TanStack Query — é o lugar que o [ADR-0013](../../../docs/adr/0013-fluxo-de-dados-no-app.md) reservou para cache de cliente nesta fase. A tabela geral continua Server Component + refresh
