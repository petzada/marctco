...
- [x] Item **Meus leads** na barra para quem atende (`ATTENDANT` e `SUPERVISOR`), rota própria. **Leads** (tabela) some do Atendente: Meus leads já é o conjunto dele (Kanban e Lista). Supervisor, Gestão e Direção continuam com a tabela. Sem Kanban global ([decisao-features-concorrentes.md](https://github.com/petzada/marctco/blob/main/decisao-features-concorrentes.md) §4)
- [x] Toggle Lista / Kanban (`{component.toggle-segmented}` do `DESIGN.md`)
- [x] Colunas = etapas do funil comercial padrão; só Oportunidades `OPEN` não mescladas, no escopo do papel ([ADR-0009](https://github.com/petzada/marctco/blob/main/docs/adr/0009-etapas-editaveis-papeis-e-status.md), [ADR-0015](https://github.com/petzada/marctco/blob/main/docs/adr/0015-perfis-de-acesso-e-escopo.md))
