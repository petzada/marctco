...
- [x] Mapeamento do [ADR-0005](https://github.com/petzada/marctco/blob/main/docs/adr/0005-idioma-codigo-en-ui-pt-br.md) **antes** da migration: `WorkspaceMember.status` (`ACTIVE | DETACHED`, default `ACTIVE`), `display_name`, `email`, `whatsapp_phone_e164`, `Tag`, `MemberTag`. Tag na oportunidade **não** nasce
...
- [x] Nunca grava `can_provision_workspace`. Nunca cria papel `OWNER` ([ADR-0021](https://github.com/petzada/marctco/blob/main/docs/adr/0021-dois-caminhos-de-nascimento-login-fechado.md))
...
- [x] Criar tag inexistente e aplicá-la ao membro são a mesma operação; várias tags no mesmo membro ([ADR-0020](https://github.com/petzada/marctco/blob/main/docs/adr/0020-tag-no-membro-define-o-time.md))
