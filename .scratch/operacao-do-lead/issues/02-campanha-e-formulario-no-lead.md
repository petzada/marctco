...
- [x] O mapeamento do [ADR-0005](https://github.com/petzada/marctco/blob/main/docs/adr/0005-idioma-codigo-en-ui-pt-br.md) ganha `Opportunity.campaign_id`, `campaign_name`, `form_id` e `form_name` **antes** da migration. A ingestão é a **única** janela em que esses valores existem: o payload bruto expira em 90 dias ([ADR-0014](https://github.com/petzada/marctco/blob/main/docs/adr/0014-copia-unica-e-retencao-do-payload.md)), e depois disso não há de onde recuperá-los
...
- [x] Retransmissão inerte **não** tem onde guardar esses campos — não sobrescreve, não apaga, não rebobina etapa nem responsável ([ADR-0007](https://github.com/petzada/marctco/blob/main/docs/adr/0007-ingestao-idempotencia.md))
...
- [x] Card mostra campanha e formulário; comparação de possível duplicado inclui esse par no conjunto de discriminadores, junto de tipo, instituição, parcela e origem — nenhum campo é prova ([plano A2](https://github.com/petzada/marctco/blob/main/docs/plano-de-construcao.md))
...
**O mapeamento veio antes da migration:** [ADR-0005](https://github.com/petzada/marctco/blob/main/docs/adr/0005-idioma-codigo-en-ui-pt-br.md) ganhou as quatro linhas (`Opportunity.campaign_id`, `campaign_name`, `form_id`, `form_name`). Os outros seis campos de atribuição do `v1` não entram. Nenhum campo monetário novo.
