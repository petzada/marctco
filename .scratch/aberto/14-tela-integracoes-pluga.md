...
- [x] Histórico recente de eventos com data, situação e erro. **Fechado pelo ticket 15**, como previsto aqui: a migration `20260811001500` deu a `integration_events` as colunas `failed_at`/`failure_reason`, o worker passou a gravar `status = FAILED` quando o BullMQ esgota as tentativas, e a tela ganhou a coluna "Erro" no histórico mais a seção "Fila morta"
