...
**Workspace**:
Tenant SaaS cuja fronteira é o **dono**: todo o grupo do mesmo dono cabe num workspace, e os leads de todas as campanhas caem numa fila única. Campanha exclusiva de uma sub-empresa **não** abre tenant — ela ganha conexão de integração própria dentro do mesmo workspace, porque separar tenant custaria reconhecer a mesma Pessoa duas vezes. Isola dados, integrações, trial e feature flags. A mesma Direção ainda pode ser dona de vários, e o caso legítimo é comprar operações que não se falam. **Outro dono é outro workspace** — não a pessoa jurídica do grupo. Lead da REAL no atendente da ACR, no tenant compartilhado, é atribuição; lead de um cliente terceiro da marctco é vazamento.
_Avoid_: Conta, tenant solto, organização Clerk, workspace por filial, workspace por campanha exclusiva, um workspace automático para o grupo inteiro, tratar a empresa que pagou o anúncio como fronteira de dados, tratar assessoria ou CNPJ como tenant, chamar ACR e REAL de “outra assessoria” quando compartilham a fila, sub-empresa como tenant ou como escopo da Gestão
...
Como o lead sai da fila e chega em quem atende, em dois níveis. A Gestão (na prática o analista de marketing) abre a fila sem dono e **atribui** o lead ao Supervisor da equipe — `ACTIVE` e com ao menos uma tag —, ou assume o card ela mesma; o Supervisor **reatribui** ao Atendente do seu time — nunca lead de outro Supervisor, que não está no time dele. Atendente nunca nasce dono direto da fila. Cada gesto é para **um** destino, um card ou vários de uma vez; em massa é o caminho preferido quando o volume pede. O lote é **parcial**: quem ainda podia ir, vai; quem já tinha dono recusa pelo nome e permanece na vista. A fila sem dono não entra no escopo do Supervisor — ver e atribuir dali é da Gestão e da Direção. Quem atende é decisão de capacidade da operação, nunca da campanha nem da empresa do grupo que pagou o anúncio.
...
**Perfil de acesso**:
O que uma pessoa responde dentro do workspace, e portanto o que ela alcança. São quatro, e nenhum a mais — **Atendente** responde só pelos leads atribuídos a ele: não vê lead de outro atendente, não vê o time, não vê a fila sem dono; **Supervisor**, pelo time (quem compartilha tag no membro, **menos os outros Supervisores**, e as oportunidades atribuídas a eles, inclusive as que estão no próprio Supervisor); **Gestão**, pela operação inteira, inclusive a fila sem dono; **Direção**, pela operação e pela conta. O escopo é aplicado no servidor, num lugar só. Isolar outro dono é tenant (RLS + workspace), não perfil — e nunca empresa do grupo.
...
_Avoid_: Tag como marca, sub-workspace, departamento como tenant, “empresa” no sentido de workspace, catálogo de equipe vazio, tela Nova equipe, copiar a tag do atendente para a oportunidade, computar o time do Supervisor a partir da oportunidade, recortar Gestão por tag, tela de taxonomia fora da Equipe

**Empresa**:
Sub-empresa do grupo (ACR, REAL) como **agrupamento de equipes**, para a Direção ler a operação por empresa. A tag aponta para ela; o membro não a carrega, porque a empresa de uma pessoa é a da equipe dela. É dimensão de leitura e nada mais: não isola dado, não decide quem alcança o quê, não roteia lead e não é coluna da Oportunidade. A quem uma venda pertence continua em aberto — campanha, quem atendeu, quem fechou e quem contabiliza podem divergir, e a forma prevista é congelar responsável, equipe e empresa no Ganho, quando o relatório existir.
_Avoid_: Business unit, filial ou marca como tenant, empresa no membro, empresa na Oportunidade, empresa em política de RLS ou em escopo de papel, campanha apontando para empresa, uma Oportunidade por empresa para o mesmo lead
...
**EnvioLead**:
Cada submissão de formulário Ads/LP recebida, idempotente pela **conexão** por onde entrou somada à origem e ao identificador que ela forneceu. A conexão está na chave porque duas landing pages numeram por conta própria: sem ela, o lead `1` da segunda LP é engolido como retransmissão do lead `1` da primeira, sem card e sem erro.
_Avoid_: Chamar de Lead — na UI, Lead é a Oportunidade; EnvioLead nunca aparece como tal para o usuário; chave idempotente sem a conexão
...
A tela de uma origem somada às conexões que ela administra: o segmento de URL, o provedor, o endereço de ingestão e o texto que difere entre as telas. **Um provedor admite várias conexões no mesmo workspace** — cada landing page e cada conta de anúncio tem nome, segredo e destino próprios —, e é isso que deixa a campanha exclusiva de uma sub-empresa entrar sem abrir tenant. Existe porque os dois lados já se separaram uma vez — a tela de landing page documentava um token que nenhuma rota sabia emitir, porque a rota da Pluga trazia o provedor fixo dentro do arquivo. Tela e rotas passam a ler o provedor do mesmo lugar. Vive na camada web; não é model nem coluna.
