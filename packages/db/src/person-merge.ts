 ...
 * re-evaluates the open cards that now belong to the canonical Pessoa.
 *
 * Exported (not just internal) because `resolveIdentityConflict` in
 * `leads.ts` needs the merge and the review's resolution audit row to land
 * in the **same** transaction — two separate commits would reopen the exact
 * window ticket 09 found and fixed for `applyIntakePlan`: a crash or a
 * concurrent retry between them leaves persons merged with the review that
 * asked for it still reading as pending, or the reverse.
 */
export async function mergePersons(
  ...
  return withAccessContext(prisma, context, (transaction) =>
    mergePersonsInTransaction(transaction, context.workspace_id, input)
  );
}

/**
 * The body of `mergePersons`, taking an already-open scoped transaction. The
 * public operation above is a thin wrapper that opens one; `leads.ts` calls
 * this directly so a Pessoa merge and the review resolution that requested it
 * commit together, never as two separate windows.
 */
export async function mergePersonsInTransaction(
  ...
  await transaction.$executeRaw`
    ...
    WHERE canonical.id = ${input.canonical_person_id}::uuid
      AND canonical.workspace_id = ${workspace_id}::uuid
      AND absorbed.id = ${input.absorbed_person_id}::uuid
      AND absorbed.workspace_id = ${workspace_id}::uuid
  `;
  ...
  await transaction.$executeRaw`
    ...
    WHERE newer.workspace_id = ${workspace_id}::uuid
      ...
      AND NOT EXISTS (
        ...
        WHERE existing.workspace_id = ${workspace_id}::uuid
